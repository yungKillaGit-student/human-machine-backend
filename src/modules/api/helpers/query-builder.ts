...
interface IGenericQueryBuilderSettings {
    tableAlias: string;
}
...
export class GenericQueryBuilder {
    ...
    settings: IGenericQueryBuilderSettings;

    constructor(settings) {
        this.settings = settings;
    }

    public getQuerySubStatements(statement: SCondition, logicalKey = 'AND'): string {
        let query = '';
        const statementKeys = Object.keys(statement);

        if (statementKeys.length > 0) {
            statementKeys.forEach((key, i) => {
                if (GenericQueryBuilder.KEYS.indexOf(key) >= 0) {
                    query += '(';
    ...
    public isStatementIncludesLogicalOperators(statement: any): boolean {
        ...
        if (Array.isArray(statement)) {
            GenericQueryBuilder.KEYS.forEach(key => statement.forEach(value => value[key] ? isInclude = true : false));
        }
        return isInclude;
    }
    ...
    public getCompareStatement(key: string, value: any): string {
        ...
        statement = `"${this.settings.tableAlias}"."${snakeCase(key)}"`;
